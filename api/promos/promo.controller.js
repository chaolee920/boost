...
exports.getPromos = function(req, res) {
    ...
    Promo.find({}).populate('creator', 'email firstName lastName roles').exec(function(err, promos) {
        if (err) {
            res.json({
                code: 402,
                message: config.DB_ERROR,
...
exports.getPromo = function(req, res) {
    ...
    Promo.find({ _id: req.params.id }).populate('event').exec(function(err, promos) {
        if (err) {
            res.json({
                code: 402,
                message: config.DB_ERROR,
